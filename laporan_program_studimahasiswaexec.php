...
if ($All == "Tidak"){ 
$result = mysqli_query($con, "SELECT program_studi.* FROM program_studi inner join mahasiswa on program_studi.Kode = mahasiswa.Program_Studi inner join sertifikasi on mahasiswa.NIM = sertifikasi.NIM where Tanggal_Daftar >= '$TanggalAwal' and Hasil_Ujian <= '$TanggalAkhir'");
} else { 
...
echo "<tr>"; 
echo "<td colspan=4>"; 
echo "<table id=laporan align=right width=100%>";  
echo "<tr>";
...
echo "</tr>";
$result2 = mysqli_query($con, "SELECT mahasiswa.* FROM mahasiswa where Program_Studi = '". $row['Kode'] ."'");
...
echo "<td><font face=Verdana color=black size=1>" . $row2['Nama'] . "</font></td>";
  ...
  echo "</font></td>";
  echo "<td><font face=Verdana color=black size=1><a href='images/" . $row2['Foto'] . "' target=_blank><img src='images/" . $row2['Foto'] . "'  width=50 height=50></a></font></td>";
echo "</tr>"; 
} 
echo "</table>"; 
} 
