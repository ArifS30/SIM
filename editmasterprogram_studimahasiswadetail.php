...
echo "<form action=editmasterprogram_studimahasiswadetailexec.php method=post>";
$result = mysqli_query($con, "SELECT * FROM mahasiswa where NIM = '". $_GET['NIM'] . "'");
while($row = mysqli_fetch_array($result))
{
echo "<td bgcolor=CCEEEE><input type=hidden name=pk value='" . $row['NIM'] . "'></td>";
echo "<tr><td bgcolor=CCCCCC><font face=Verdana color=black size=1>&nbsp;&nbsp;NIM&nbsp;&nbsp;</font></td>";
echo "<td bgcolor=CCEEEE>&nbsp;&nbsp;<input type=text  class='form-control' name='NIM' value='" . $row['NIM'] . "'></td>";
echo "<tr><td bgcolor=CCCCCC><font face=Verdana color=black size=1>&nbsp;&nbsp;Nama&nbsp;&nbsp;</font></td>";
echo "<td bgcolor=CCEEEE><input type=text  class='form-control' name='Nama' value='" . $row['Nama'] . "' size = 100></td></tr>";
