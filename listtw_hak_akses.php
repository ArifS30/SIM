...
<?php
//cek otoritas
$q = "SELECT * FROM tw_hak_akses where tabel='tw_hak_akses' and user = '". $_SESSION['Email'] ."' and listData='1'";
$r = mysqli_query($con, $q);
if ( $obj = @mysqli_fetch_object($r) )
 {
?>
<?php
$menu=mysqli_query($con, "SELECT * from tw_hak_akses");
while($rowmenu = mysqli_fetch_array($menu))
{
    echo "<option value=". $rowmenu['id'] .">". $rowmenu['tabel']."</option>";
}
echo "<br><font face=Verdana color=black size=1>tw_hak_akses</font><br><br>";
echo "<a href=inserttw_hak_akses.php><button type='button' class='btn btn-light'><font face=Verdana color=black size=1><i class='fa fa-plus'></i>&nbsp;Insert</font></button></a>";
echo "&nbsp;&nbsp;<a href='printtw_hak_akses.php' target=_blank><button type='button' class='btn btn-light'><font face=Verdana color=black size=1><i class='fa fa-print'></i>&nbsp;Print</font></button></a>";
//cari tabel
echo "<br><br><form action=listtw_hak_akses.php method=post>
...
while($row = mysqli_fetch_array($result))
  ...
  echo "<td><a class=linklist href=viewtw_hak_akses.php?id=".$row['id']."><button type='button' class='btn btn-warning'><font face=Verdana size=1><i class='fa fa-check'></i></font></button></a></td>";
  echo "<td><a class=linklist href=edittw_hak_akses.php?id=".$row['id']."><button type='button' class='btn btn-primary'><font face=Verdana size=1><i class='fa fa-edit'></i></font></button></a></td>";
  echo "<td><a class=linklist href=deletetw_hak_akses.php?id=".$row['id']." onclick=\"return confirm('Are you sure you want to delete this data?')\"><button type='button' class='btn btn-danger'><font face=Verdana size=1><i class='fa fa-trash'></i></font></button></a></td>";
...
echo "</div>";
	...
	echo "<font size=2 face=Verdana color=#FF0000>Data tw_hak_akses not found - try again!</font><br><br>";
}
}
...
$result = mysqli_query($con, "SELECT * FROM tw_hak_akses LIMIT $posisi,$batas");
echo "<div class='table-responsive'> "; 
echo "<table class='table table-striped'>"; 
$firstColumn = 1;
$warna = 0;
...
$tampil2 = mysqli_query($con, "SELECT * FROM tw_hak_akses");
