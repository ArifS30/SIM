...
<?php
//cek otoritas
$q = "SELECT * FROM tw_hak_akses where tabel='mahasiswa/sertifikasi' and user = '". $_SESSION['Email'] ."' and detailData='1'";
...
echo "<tr><td align=left><a href=listmastermahasiswasertifikasi.php><button type='button' class='btn btn-warning'><font face=Verdana size=1><i class='fa fa-check'></i>&nbsp;Back</font></button></a></td></tr>";
echo "</table>";
echo "</div>";
echo "<br><br>";
echo "<font face=Verdana color=black size=1>sertifikasi</font><br>";
if(isset($_GET['NIM'])){echo "<a href=insertmastermahasiswasertifikasidetail.php?NIM=". mysqli_real_escape_string($con, $_GET['NIM']) ."><button type='button' class='btn btn-light'><font face=Verdana size=1><i class='fa fa-plus'></i>&nbsp;Insert</font></button></a><br><br>";}
...
if(isset($_GET['NIM'])){$result = mysqli_query($con, "SELECT * FROM sertifikasi where sertifikasi.NIM= '".mysqli_real_escape_string($con, $_GET['NIM'])."' LIMIT $posisi,$batas");}
echo "<div class='table-responsive'> "; 
echo "<table class='table table-striped'>"; 
$firstColumn = 1;
$warna = 0;
...
<td><font face=Verdana color=black size=1><b>Tanggal_Ujian</b></font></td>
<td><font face=Verdana color=black size=1><b>Hasil_Ujian</b></font></td>
...
$firstColumn = 0;
  ...
  if(isset($_GET['NIM'])){echo "<td><a class=linklist href=viewmastermahasiswasertifikasidetail.php?id=".$row['id']."&NIM=". mysqli_real_escape_string($con, $_GET['NIM']) ."><button type='button' class='btn btn-warning'><font face=Verdana size=1><i class='fa fa-check'></i></font></button></a></td>";}
  if(isset($_GET['NIM'])){echo "<td><a class=linklist href=editmastermahasiswasertifikasidetail.php?id=".$row['id']."&NIM=". mysqli_real_escape_string($con, $_GET['NIM']) ."><button type='button' class='btn btn-primary'><font face=Verdana size=1><i class='fa fa-edit'></i></font></button></a></td>";}
  if(isset($_GET['NIM'])){echo "<td><a class=linklist href=deletemastermahasiswasertifikasidetail.php?id=".$row['id']."&NIM=". mysqli_real_escape_string($con, $_GET['NIM']) ." onclick=\"return confirm('Are you sure you want to delete this data?')\"><button type='button' class='btn btn-danger'><font face=Verdana size=1><i class='fa fa-Trash'></i></font></button></a></td>";}
...
if(isset($_GET['NIM'])){$tampil2 = mysqli_query($con, "SELECT * FROM sertifikasi where sertifikasi.NIM='".mysqli_real_escape_string($con, $_GET['NIM'])."'");}
