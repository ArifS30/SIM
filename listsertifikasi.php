...
?>
<?php
echo "<br><font face=Verdana color=black size=1>sertifikasi</font><br><br>";
echo "<a href=insertsertifikasi.php><button type='button' class='btn btn-light'><font face=Verdana color=black size=1><i class='fa fa-plus'></i>&nbsp;Insert</font></button></a>";
echo "&nbsp;&nbsp;<a href='printsertifikasi.php' target=_blank><button type='button' class='btn btn-light'><font face=Verdana color=black size=1><i class='fa fa-print'></i>&nbsp;Print</font></button></a>";
//cari tabel
echo "<br><br><form action=listsertifikasi.php method=post>
...
$dd = "SELECT * FROM sertifikasi where "  . $_POST["select"] . " like '%" . $cari . "%'";
...
while($row = mysqli_fetch_array($result))
  ...
  echo "<td><a class=linklist href=viewsertifikasi.php?id=".$row['id']."><button type='button' class='btn btn-warning'><font face=Verdana size=1><i class='fa fa-check'></i></font></button></a></td>";
  echo "<td><a class=linklist href=editsertifikasi.php?id=".$row['id']."><button type='button' class='btn btn-primary'><font face=Verdana size=1><i class='fa fa-edit'></i></font></button></a></td>";
  echo "<td><a class=linklist href=deletesertifikasi.php?id=".$row['id']." onclick=\"return confirm('Are you sure you want to delete this data?')\"><button type='button' class='btn btn-danger'><font face=Verdana size=1><i class='fa fa-trash'></i></font></button></a></td>";
  echo "</tr>";
  }
echo "</table><br><br>";
echo "</div>";
} else {
	echo "<font size=2 face=Verdana color=#FF0000>Data sertifikasi not found - try again!</font><br><br>";
}
}
...
$result = mysqli_query($con, "SELECT * FROM sertifikasi LIMIT $posisi,$batas");
echo "<div class='table-responsive'> "; 
echo "<table class='table table-bordered'>"; 
$firstColumn = 1;
$warna = 0;
...
$tampil2 = mysqli_query($con, "SELECT * FROM sertifikasi");
